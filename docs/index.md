# Projeto de Interacao Humano-Computador

## Apresentação

Este site documenta de forma rastreável e acessível o projeto acadêmico de Interação Humano-Computador (IHC) desenvolvido pelo Grupo 03. O espaço reúne o planejamento estratégico, as decisões de design, os artefatos produzidos, as atas de reunião e as evidências de contribuição da equipe.

## Sites avaliados

| Site            | Contexto                           | Principais observacoes                                                                                                                                            | Evidencia                                                                                                                                                |
| --------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Portal do GDF   | Média — portal institucional       | Poucas reclamações sobre a usabilidade do site em si; a maioria das queixas é sobre qualidade de serviços públicos (iluminação,benefícios), não sobre a interface | [Baixar Relatório em PDF](./planejamento/sites%20avaliados/Henrique%20Schneider%20Fernandes%20da%20Rosa/ProjetoParteIndividualMetodoeAvalição.pdf)       |
| Meu SUS Digital | Saúde digital acessível            | Falha ao editar dados do usuário                                                                                                                                  | [Baixar Relatório em PDF](./planejamento/sites%20avaliados/Eduardo%20Ribeiro%20Gomes%20da%20Silva/avaliaoMeuSUSDigital.pdf)                              |
| Detran DF       | Serviços públicos de trânsito      | Falhas de validação no login e fluxo de agendamento confuso                                                                                                       | [Baixar Relatório em PDF](./planejamento/sites%20avaliados/João%20Vitor%20Tavares%20de%20Sá%20Lima/PlojetoParteIndividualMetodoeAvalição.pdf)            |
| SouGov          | Portal do servidor público federal | Problemas de interação                                                                                                                                            | [Baixar Relatório em PDF](./planejamento/sites%20avaliados/João%20Vitor%20Tavares%20de%20Sá%20Lima/SouGovMetodoeAvalição.pdf)                            |
| MeuINSS         | Previdência social brasileira.     | fragmentação do ecossistema                                                                                                                                       | [Baixar Relatório em PDF](<./planejamento/sites%20avaliados/Camile%20Barbosa%20Gonzaga%20de%20Oliveira/PlojetoParteIndividualMetodoeAvalição%20(1).pdf>) |

## Site escolhido

**Site selecionado:** [Detran-DF](https://www.detran.df.gov.br/)

## Motivação

O portal foi escolhido democraticamente pelo grupo por apresentar barreiras severas de usabilidade, falhas comunicativas em formulários de consulta e inconsistências no fluxo de emissão de taxas e documentos, impactando negativamente a satisfação do cidadão.

## Criterios de escolha

- Relevancia do problema de interacao para o publico-alvo.
- Possibilidade de observar e documentar evidencias de usabilidade.
- Viabilidade de propor melhorias no escopo e no prazo da disciplina.
- Diversidade de fluxos, conteudos e componentes para analise.
- Disponibilidade de fontes e referencias para fundamentacao.

## Agradecimentos

Agradecemos à equipe do Grupo 03 pela dedicação na análise dos diferentes portais avaliados e na seleção colaborativa do Detran-DF, cujas barreiras de usabilidade e falhas em formulários motivaram o desenvolvimento deste projeto acadêmico de Interação Humano-Computador (IHC).

## Uso de Inteligência Artificial

O uso de ferramentas de Inteligência Artificial Generativa foi empregado exclusivamente para o refinamento ortográfico, padronização da linguagem acadêmica e formatação da estrutura Markdown (tabelas e tópicos) deste documento. A escolha do método, a argumentação de projeto e as validações técnicas são de inteira responsabilidade e autoria intelectual da equipe.
